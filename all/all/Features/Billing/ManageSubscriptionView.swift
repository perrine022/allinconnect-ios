...
struct ManageSubscriptionView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            print("═══════════════════════════════════════════════════════════")
            print("📱 [ManageSubscriptionView] onAppear - DÉBUT")
            print("═══════════════════════════════════════════════════════════")
            Task {
                await viewModel.loadSubscriptionStatus()
                await viewModel.loadSubscriptionDetails()
                // loadUserLightData() est déjà appelé dans loadSubscriptionStatus()
                // Vérifier que cardValidityDate est bien chargé
                if let cardValidityDate = viewModel.cardValidityDate {
                    let dateFormatter = DateFormatter()
                    dateFormatter.dateStyle = .medium
                    dateFormatter.timeStyle = .short
                    dateFormatter.locale = Locale(identifier: "fr_FR")
                    print("✅ [ManageSubscriptionView] cardValidityDate disponible après chargement: \(dateFormatter.string(from: cardValidityDate))")
                } else {
                    print("❌ [ManageSubscriptionView] cardValidityDate toujours nil après chargement")
                }
                print("═══════════════════════════════════════════════════════════")
            }
        }
